            ...
            .addInterceptor { chain ->
                ...
                request = if (hasNetwork(context)!!) {
                    request.newBuilder().header("Cache-Control", "public, max-age=" + 5)
                        .build() //Will discard cache if older than 5 seconds and hasNetwork Connection
                } else request.newBuilder().header(
                    "Cache-Control",
                    "public, only-if-cached, max-stale=" + 60 * 60 * 24 * 7
                ).build() //If no network, will use cache up to 7 days old, otherwise discard and throw error
